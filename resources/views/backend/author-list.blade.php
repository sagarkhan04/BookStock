            ...
            <table class="w-full">
              ...
              <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-6 py-4">
                    <a href="{{ route('authors.show', $author->id) }}" class="font-medium text-indigo-600 hover:text-indigo-800">{{ $author->name }}</a>
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-600">{{ Str::limit($author->bio, 50) ?? '—' }}</td>
                  <td class="px-6 py-4 text-sm text-gray-600">{{ $author->books_count ?? 0 }}</td>
                  <td class="px-6 py-4">
                    <form method="POST" action="{{ route('authors.toggle-status', $author->id) }}" style="display: inline;">
                      @csrf
                      <button type="submit" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium transition-colors {{ $author->is_active ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' }}">
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center space-x-2">
                      <a href="{{ route('authors.edit', $author->id) }}" class="text-indigo-600 hover:text-indigo-800 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                        </svg>
                      </a>
                      <form method="POST" action="{{ route('authors.destroy', $author->id) }}" style="display: inline;" onsubmit="return confirm('Are you sure?')">
                        @csrf
                        @method('DELETE')
                ...
                <tr>
                  <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                    ...
                    <a href="{{ route('authors.create') }}" class="text-indigo-600 hover:text-indigo-800 mt-2 inline-block">Create the first author</a>
                  </td>
                </tr>
                @endforelse
              </tbody>
