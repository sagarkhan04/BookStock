...
<html lang="en">
  <head>
    ...
    <title>{{ $author->name }} | Interactive Cares</title>
        ...
        <header class="bg-white shadow-sm sticky top-0 z-20">
          <div class="px-6 lg:px-8 py-4 flex items-center justify-between">
            <div class="flex items-center space-x-4">
              ...
              <div>
                <h1 class="text-xl font-bold text-gray-800">{{ $author->name }}</h1>
                <p class="text-sm text-gray-500">Author Details</p>
              </div>
            </div>

            <div class="flex items-center space-x-3">
              <a href="{{ route('authors.edit', $author->id) }}" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">
                Edit
              </a>
          ...
          <div class="bg-white rounded-xl shadow overflow-hidden">
            ...
            <div class="p-6">
              <div class="mb-8">
                ...
                @else
                  <p class="text-gray-500">No biography available.</p>
                @endif
                <p class="text-sm text-gray-500 mt-4">Created: {{ \Carbon\Carbon::parse($author->created_at)->format('M d, Y') }}</p>
              </div>

              <h3 class="text-lg font-semibold text-gray-900 mb-4">Books by this Author</h3>
              @if ($books->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  @foreach ($books as $book)
                    <a href="{{ route('books.show', $book->id) }}" class="group border border-gray-200 rounded-lg p-4 hover:shadow-md hover:border-indigo-300 transition-all">
                      <div class="text-sm text-gray-500 mb-2">{{ $book->category_name ?? 'Uncategorized' }}</div>
